...
export default function About() {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
          ([entry]) => {
            const ratio = entry.intersectionRatio; // from 0 to 1
            setBannerOpacity(ratio); // save for render or styling
            textBox.style.opacity = ratio.toFixed(2); // apply fade
          },
          {
            threshold: Array.from({ length: 101 }, (_, i) => i / 100), // trigger on every 1%
          }
        );
