...
export default function About() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (!banner || !textBox) return;
    
            const bannerRect = banner.getBoundingClientRect();
            const bannerCenter = bannerRect.top + bannerRect.height / 2;
            const screenCenter = window.innerHeight / 2;
    
            const distanceFromCenter = Math.abs(bannerCenter - screenCenter);
            const fadeRange = window.innerHeight / 2; // how far from center it starts fading
    
            // Map distance to opacity: 0 opacity when far, 1 when centered
